...
class Benchmark(threading.Thread):
    ...
    def run(self):
        ...
        process = psutil.Process(17875)  # give pid
    ...
    def stop(self):
        self.running = False
        csv_file = "base-250p-new" + str(datetime.now().strftime("%H%M%S")) + ".csv"
...
def main():
    ...
    for i in range(20):
        ...
        p.run("cd /local/jcs670/mcserver/yardstick && (nohup java -jar target/yardstick-1.0.1-jar-with-dependencies.jar -e 4 -h 10.149.0.72 -Ebots=250 -Eduration=600 &> /dev/null < /dev/null &) && /bin/true")
